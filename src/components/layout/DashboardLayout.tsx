...
export function Dashboard({ children }: { children: React.ReactNode }) {
    ...
    const hideHeader = /^\/ssh\/sftp\/(edit|editor)/.test(location.pathname);
    ...
    return hideHeader ? (
        <div className="flex flex-col overflow-hidden bg-[#0A0A0A] transition-all duration-300 ease-in-out" 
        style={{ height: "100vh" }}>
            {children}
        </div>
    ) : (
            ...
            >
                ...
                <div className="absolute left-1/2 -translate-x-1/2 flex items-center">
                    <label className="flex items-center gap-1.5 cursor-pointer select-none text-xs text-muted-foreground">
                        ...
                        >
                            <span
                                className={`inline-block h-3 w-3 rounded-full bg-white shadow transition-transform duration-200 mt-0.5
                    ...
                    <span className="flex items-center gap-1 text-xs text-muted-foreground whitespace-nowrap">
                        Made by{" "}
                        <span className="font-semibold text-foreground">Enjoys</span> with
